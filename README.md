...
There are some special cases where `polyscribe` must replace data in your Markdown file with data from the Canvas course retreived via the API. Below, you'll find explanations for elements that have non-standard rendering rules:

#### Anchor Elements

In Markdown, you create an anchor element like so:

```md
[Display text](URL)
```

If you want to use an anchor element to create a link to an external webpage (e.g., `https://google.com`), you should write the Markdown anchor exactly as you normally would. However, if you want to use an anchor element to create a link to an _internal Canvas page_, you should reference the Markdown file rather than the Canvas URL. For example, if I had a discussion board element in the `modules` directory at the path `introduction/discussion.md`, I would link to it in other Canvas elements like so:

```md
[Discussion board](introduction/discussion.md)
```

If the file containing the anchor element is in the same directory as the element you'd like to link to, you can use a `./` relative path like so:

```md
[Discussion board](./discussion.md)
```

URLs containing `../` are not supported.

---

In addition to standard Markdown syntax and the special cases above, `polyscribe-canvas` supports several "directives" which provide enhanced user interfaces for common HTML snippets authors often wish to include in their course elements. You can read about directives in the **Directives** section below.
